...
import { Employe, Departement, Formation, Stagiaire, InscriptionFormation } from '../models';
...
const seed = async () => {
  ...
  // 1. Départements
  const depts = await Departement.bulkCreate([
  ...
  logger.info(`✅ ${depts.length} départements créés`);

  // 2. Employés (6 rôles différents)
  const employesData = [
    // Direction RH
    { nom: 'Mansouri', prenom: 'Fatma', email: 'rh@cni.tn', motDePasse: 'password123', role: 'Direction_RH', departementId: 3, poste: 'Directrice RH', matricule: 'CNI-001' },
    // Manager DEV
  ...
  const employesHashed = await Promise.all(
    employesData.map(async (e) => ({
      ...e,
      motDePasse: await bcrypt.hash(e.motDePasse, 12),
    }))
  );

  const employes = await Employe.bulkCreate(employesHashed as any, {
    updateOnDuplicate: ['nom', 'prenom', 'motDePasse', 'role', 'departementId', 'poste', 'matricule'],
  });
  logger.info(`✅ ${employes.length} employés créés`);

  // Récupérer les IDs fiables après upsert
  const employesDb = await Employe.findAll({
    where: {
      email: [
        'rh@cni.tn',
        'manager.dev@cni.tn',
  ...
  const mustId = (email: string) => {
    const id = employeByEmail.get(email)?.id;
    if (!id) throw new Error(`Employé introuvable après seeding: ${email}`);
    return id;
  };

  // Mettre à jour responsables départements
  await Departement.update({ responsableId: mustId('manager.dev@cni.tn') }, { where: { code: 'DEV' } });
  await Departement.update({ responsableId: mustId('manager.inf@cni.tn') }, { where: { code: 'INF' } });
  await Departement.update({ responsableId: mustId('rh@cni.tn') }, { where: { code: 'RH' } });
  ...
  logger.info(`✅ ${formations.length} formations créées`);

  // 4. Stagiaires
  const stagiaires = await Stagiaire.bulkCreate([
    {
  ...
  logger.info(`✅ ${stagiaires.length} stagiaires créés`);

  // 5. Inscriptions formations
  await InscriptionFormation.bulkCreate([
  ...
  logger.info('✅ Inscriptions créées');
...
seed().catch(err => {
  logger.error('❌ Erreur seeding:', err);
  process.exit(1);
});
