...
import { registerEmailDeliver, enqueueEmailDelivery } from '../services/emailQueue.service';
...
export const initEmailTransporter = async (): Promise<void> => {
  ...
  if (useSmtp) {
    ...
    try {
      ...
      logger.info(`📧 SMTP actif et vérifié : ${cfg.host}:${cfg.port} (${cfg.user})`);
    } catch (e) {
      logger.error(`❌ Échec connexion SMTP (${cfg.host}:${cfg.port})`, e);
      throw e;
    }
    return;
  }
  ...
  if (process.env.NODE_ENV === 'production' && !canSmtp) {
    logger.warn('⚠️ Production : variables SMTP incomplètes — utilisation du transport de secours.');
  }
  ...
  try {
    ...
    logger.info(`📧 Email de test Ethereal : ${testAccount.user}`);
    logger.info(`🔑 Mot de passe Ethereal : ${testAccount.pass}`);
    logger.info('📬 Prévisualisation : https://ethereal.email');
  } catch {
    logger.warn('⚠️ Ethereal indisponible — emails loggés en console');
    transporter = {
      sendMail: async (options: nodemailer.SendMailOptions) => {
        logger.info('📧 EMAIL (console):');
        logger.info(`  À: ${options.to} | Sujet: ${options.subject}`);
...
export async function deliverEmailNow(options: SendEmailOptions): Promise<void> {
  if (!transporter) {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (previewUrl) {
        logger.info(`📧 Email envoyé - Prévisualisation : ${previewUrl}`);
      }
    }

    logger.info(`✉️ Email envoyé à : ${options.to} | Sujet : ${options.subject}`);
  } catch (error) {
...
registerEmailDeliver(deliverEmailNow);

/**
 * Envoie un email (file mémoire, Redis si REDIS_URL, ou synchrone si sync / pièces jointes).
 */
export const sendEmail = async (options: SendEmailOptions, opts?: { sync?: boolean }): Promise<void> => {
  await enqueueEmailDelivery(options, { sync: opts?.sync });
};
...
export const emailTemplates = {
  ...
  baseTemplate: (title: string, content: string, ctaText?: string, ctaUrl?: string) => `
    <!DOCTYPE html>
  ...
  confirmationCandidature: (nom: string, prenom: string, departement: string) =>
    emailTemplates.baseTemplate(
      `Confirmation de votre candidature de stage`,
      `<p>Bonjour <strong>${prenom} ${nom}</strong>,</p>
       ...
       <p>Merci pour votre intérêt pour CNI !</p>`,
      'Suivre ma candidature',
      `${process.env.FRONTEND_URL}/candidat/suivi`
    ),

  // Notification RH nouvelle candidature
  notificationRH: (nom: string, prenom: string, departement: string, email: string) =>
    emailTemplates.baseTemplate(
      `Nouvelle candidature de stage reçue`,
      `<p>Une nouvelle candidature de stage a été soumise :</p>
      ...
      `${process.env.FRONTEND_URL}/rh/candidatures`
    ),

  /** Code OTP (vérification e-mail) */
  otpCode: (code: string, purpose: 'candidature' | 'register', minutesValid: number) =>
    emailTemplates.baseTemplate(
